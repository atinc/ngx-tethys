...
* **button:** add `thyAppearance` and simplify `ThyButtonType` #TINFR-3855 — `fill` / `outline` / `link` 与颜色正交；旧复合 type（如 `outline-primary`、`link-secondary`）改为 `thyAppearance` + `thyButton`；库内正式用法已迁移；`ng update` `migration-v22` 含 Button Appearance Schematics。破坏性对照：`link-secondary` → `thyAppearance="link" thyButton="default"`；`ThyButtonGroup` 的 `outline-*` 仍为独立 API，本版未改。
