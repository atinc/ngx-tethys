...
(function() {
    ...
    InlineLexer.prototype.output = function(src) {
        ...
        while (src) {
            ...
            if ((this.options.link && (cap = this.rules.reflink.exec(src))) || (cap = this.rules.nolink.exec(src))) {
                src = src.substring(cap[0].length);
    ...
    Renderer.prototype.image = function(href, title, text, origin) {
        if (this.options.isImageDefault) {
            ...
            if (this.options.isImgPreview) {
                out +=
                    '<a class="fancybox attachment-thumb ng-scope" href="' +
                    href +
                    '" data-fancybox-group="filegroup-detail-img" data-fancybox-type="image">';
    ...
    Renderer.prototype.wthexcolor = function(color, prefix) {
        ...
        if (this.options.wthexcolorRender.className) {
            out +=
                color +
                '<div class="' +
                this.options.wthexcolorRender.className +
                '" style="background:' +
                color +
                '"></div>';
        } else {
    ...
    Renderer.prototype.wtentity = function(_origin, _type, _id, _name, prefix) {
        ...
        out +=
            ...
            _origin +
            '" class="hand ' +
            this.options.wtentityRender.className +
            ' marked-entity"><i class="wtf wtf-link-' +
            _type +
            '"></i>' +
            _name +
            '</a>';
    ...
    Renderer.prototype.wtemoji = function(text, prefix) {
        ...
        if (emoji) {
            out +=
                '<img class="' +
                this.options.wtemojiRender.className +
                '"' +
                ' title="' +
                text +
                '" alt="' +
                emoji.name +
                '" src="' +
                this.options.wtemojiRender.getImageSrc(emoji) +
                '" />';
        } else {
            out += text;
        }
    ...
    Parser.prototype.tok = function() {
        switch (this.token.type) {
            ...
            case 'code': {
                return this.renderer.code(this.token.text, this.token.lang, this.token.escaped, this.token.origin);
            }
            case 'table': {
                var header = '',
                    body = '',
            ...
            case 'html': {
                var html =
                    !this.token.pre && !this.options.pedantic ? this.inline.output(this.token.text) : this.token.text;
                return this.renderer.html(html, this.token.origin);
            }
            case 'paragraph': {
                return this.renderer.paragraph(this.inline.output(this.token.text), this.token.origin);
            }
            case 'text': {
    ...
    liteMarked.options = liteMarked.setOptions = function(opt) {
