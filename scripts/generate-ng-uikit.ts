...
function buildDistPathsForNgUIKitBuild(): Record<string, string[]> {
    const paths: Record<string, string[]> = {};

    const tethysPkgPath = path.join(rootDir, 'dist/tethys/package.json');
    if (fs.existsSync(tethysPkgPath)) {
        const pkg = fs.readJsonSync(tethysPkgPath) as { exports?: Record<string, unknown> };
        paths['ngx-tethys'] = ['../../dist/tethys/types/ngx-tethys'];
        for (const key of Object.keys(pkg.exports ?? {})) {
            if (!key.startsWith('./') || key === './package.json' || key.includes('*')) {
                continue;
            }
            const sub = key.slice(2);
            paths[`ngx-tethys/${sub}`] = [`../../dist/tethys/types/ngx-tethys-${sub.replace(/\//g, '-')}`];
        }
    }

    const cdkPkgPath = path.join(rootDir, 'dist/cdk/package.json');
    if (fs.existsSync(cdkPkgPath)) {
        const pkg = fs.readJsonSync(cdkPkgPath) as { exports?: Record<string, unknown> };
        paths['@tethys/cdk'] = ['../../dist/cdk/types/tethys-cdk'];
        for (const key of Object.keys(pkg.exports ?? {})) {
            if (!key.startsWith('./') || key === './package.json' || key.includes('*')) {
                continue;
            }
            const sub = key.slice(2);
            paths[`@tethys/cdk/${sub}`] = [`../../dist/cdk/types/tethys-cdk-${sub.replace(/\//g, '-')}`];
        }
    }
...
function writeLibraryScaffold(outputDir: string, config: NgUIKitConfig) {
    const ngPackage = {
        $schema: '../../node_modules/ng-packagr/ng-package.schema.json',
        dest: '../../dist/ng-uikit',
        lib: { entryFile: 'public-api.ts' }
    };
    fs.writeFileSync(path.join(outputDir, 'ng-package.json'), `${JSON.stringify(ngPackage, null, 2)}\n`);
    ...
    const tsconfigLib = {
        ...
        compilerOptions: {
            baseUrl: '.',
            outDir: '../../out-tsc/ng-uikit',
            declaration: true,
            declarationMap: true,
            inlineSources: true,
            types: [],
            paths: buildDistPathsForNgUIKitBuild()
        },
        exclude: ['**/*.spec.ts'],
    ...
    fs.writeFileSync(path.join(outputDir, 'tsconfig.lib.json'), `${JSON.stringify(tsconfigLib, null, 2)}\n`);
    ...
    fs.writeFileSync(path.join(outputDir, 'tsconfig.lib.prod.json'), `${JSON.stringify(tsconfigProd, null, 2)}\n`);
}
...
function main() {
    ...
    writeLibraryScaffold(outputDir, config);
