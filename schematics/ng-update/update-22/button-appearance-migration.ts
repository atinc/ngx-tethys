...
import { isButtonAppearanceTarget, openingTagHasAttribute } from '../template-attribute-utils';
...
function hasLinkDangerWeakBoundType(tag: string): boolean {
    return LINK_DANGER_WEAK_BOUND_PATTERNS.some(pattern => {
...
function collectOpenTagEdits(tag: string, tagOffset: number): RelativeTemplateEdit[] {
    ...
    if (
        /\bthyButton="link-danger-weak"/.test(tag) ||
        /\bthyType="link-danger-weak"/.test(tag) ||
        /\bthy-button="link-danger-weak"/.test(tag)
    ) {
        return collectLinkDangerWeakTextEdits(tag, tagOffset);
    }

    if (hasLinkDangerWeakBoundType(tag)) {
    ...
    edits.push(
        ...createCompoundAttributeReplacement(
            tagOffset,
            primaryAttribute,
            mapped
        )
