...
function isTagElement(element: TmplAstElement): boolean {
    ...
    return (
        element.attributes.some(attribute => attribute.name === 'thyTag') ||
        element.inputs.some(input => input.name === 'thyTag')
    );
}

export function collectTagWeakFillEdits(content: string, filePath = 'test.html'): RelativeTemplateEdit[] {
...
export class TagAppearanceMigration extends Migration<UpgradeData> {
    ...
    override visitTemplate(template: ResolvedResource): void {
        applyRelativeTemplateEdits(
            this,
            template,
            collectTagWeakFillEdits(template.content, template.filePath)
        );
    }
}
