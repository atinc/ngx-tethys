...
export class DatePopup implements OnChanges, OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.defaultPickerValue && !hasValue(this.value)) {
            ...
            this.value = makeValue(value, this.isRange, this.tz);
        }
            ...
            if (this.innerShortcutPresets.length) {
                ...
                if (this.isRange) {
                    this.innerShortcutPresets.forEach((preset: ThyShortcutPreset) => {
                        ...
                        const beginTime: number = new TinyDate(startOfDay(begin), this.tz).getTime();

                        const end: number | Date = getShortcutValue(preset.value[1]);
                        const endTime: number = new TinyDate(endOfDay(end), this.tz).getTime();
                    ...
                    this.innerShortcutPresets.forEach((preset: ThyShortcutPreset) => {
                        ...
                        const singleTime: number = new TinyDate(singleValue, this.tz).getTime();
    ...
    initDisabledDate(): void {
        ...
        if (this.minDate) {
            ...
            minDate = new TinyDate(value as Date, this.tz);
        }
        if (this.maxDate) {
            const { value } = transformDateValue(this.maxDate);
            maxDate = new TinyDate(value as Date, this.tz);
        }
    ...
    onSelectTime(value: TinyDate, partType?: RangePartType): void {
        if (this.isRange) {
            ...
            this.setValue(new TinyDate(value.nativeDate, this.tz));
        }
    }
    ...
    private getMinTinyDate() {
        return this.minDate ? new TinyDate(transformDateValue(this.minDate).value as Date, this.tz) : null;
    }

    private getMaxTinyDate() {
        return this.maxDate ? new TinyDate(transformDateValue(this.maxDate).value as Date, this.tz) : null;
    }
    ...
    private setValue(value: CompatibleValue): void {
        ...
        console.log('setValue--------', (this.value as TinyDate).getDate());
        console.log('setValue--------', (this.value as TinyDate).nativeDate);
        console.log('setValue--------', (this.value as TinyDate).getTime());
        this.isDisableTimeConfirm();
    }
    ...
    private normalizeRangeValue(value: TinyDate[], mode: ThyPanelMode = 'month'): TinyDate[] {
        ...
        const newStart = start || new TinyDate(undefined, this.tz);
        let newEnd = end;
    ...
    private isDisableTimeConfirm() {
        ...
        const date: TinyDate = this.value ? (this.value as TinyDate) : new TinyDate(undefined, this.tz);
        const minDate: TinyDate = this.getMinTinyDate();
        const maxDate: TinyDate = this.getMaxTinyDate();
        ...
        if (helpers.isArray(value)) {
            ...
            if (begin && end) {
                this.selectedValue = this.getSelectedShortcutPreset([
                    new TinyDate(begin, this.tz).startOfDay(),
                    new TinyDate(end, this.tz).endOfDay()
                ]) as TinyDate[];
            ...
            const zonedTime = this.createInZoneTime(
                new TinyDate(getShortcutValue(value), this.tz),
                originDate?.getHours() ?? 0,
                originDate?.getMinutes() ?? 0,
                originDate?.getSeconds() ?? 0
            );
            const singleTinyDate: TinyDate = this.updateHourMinute(new TinyDate(zonedTime, this.tz));
